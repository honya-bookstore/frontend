...
export default function SliderItem({ book }: SliderItemProps) {
    return (
        <div className={'flex flex-col md:flex-row items-center justify-between gap-30'}>
            <div className={'max-w-lg text-center md:text-left gap-10 flex flex-col'}>
                <h2 className={'font-prata text-5xl line-clamp-2 py-2'}>
                    {book.title}
                </h2>
                <span className={'font-normal text-md font-plus-jakarta-sans opacity-50 line-clamp-3 tracking-wide '}>
                    {book.description}
                </span>
                <Button
                    variant={'outline'}
            ...
            <Image src={`/${book.coverImageUrl}`} alt={book.title} width={250} height={250} loading={"eager"} className={'shadow-xl my-8'} />
